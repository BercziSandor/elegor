# https://github.com/walkernr/N-Body/blob/master/nbody.py
...
class celestial(sphere):
    def __init__(self, radius=1e5, mass=1e20, position=vector(0., 0., 0.),
...
def collision(bodies, body1, body2, dt):
    ...
    if collision == True:
        if body1.getmass() >= body2.getmass():
            ...
            body1.setvelocity(
                (body1.getmass() * body1.getvelocity() + body2.getmass() * body2.getvelocity()) / (
...
def newton(bodies, dt):
    ...
    colors = [color.orange, color.blue, color.green, color.magenta, color.red, color.cyan,
              color.yellow]
    while True:
        if scene.kb.keys:
            k = scene.kb.getkey()
            ...
            if k == 'a':
                ...
                while x < 2 * bodies[0].radius and x > -2 * bodies[0].radius and z < 2 * bodies[
                    0].radius and z > -2 * bodies[0].radius:
                    x = randint(int(-8 * bodies[0].radius), int(8 * bodies[0].radius))
                    z = randint(int(-8 * bodies[0].radius), int(8 * bodies[0].radius))
                ...
                for object in bodies:
                    if target != object:
                        if collision(bodies, target, object, dt) == true:
                            ...
                            target.acceleration = G * object.getmass() / mag2(
                                object.pos - target.pos)
                            target.accelerationvector = target.acceleration * target.unitvector
                            target.setacceleration(
                                target.getacceleration() + target.accelerationvector)
...
scene = display(title="Solar System", width=600, height=600, range=2e5, forward=(-1, -1, -1),
