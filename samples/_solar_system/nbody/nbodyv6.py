...
# https://github.com/walkernr/N-Body
...
# scene = display(title="Solar System", width=600, height=600, range=1e3, forward=(-1, -1, -1),
#                 autoscale=false, ambient=0)
...
starstatus = [True]
objects = [sphere(radius=sqrt(starmass[0] * 3 / 4 / stardensity[0] / pi), pos=starposition[0],
                  make_trail=True, interval=10, color=color.yellow)]
...
while 1:
    if running:
        ...
        for ij in collisionlist:
            ...
            newposition = (position[i] * mass[i, 0] + position[j] * mass[j, 0]) / (
                    mass[i, 0] + mass[j, 0])
            newmass = mass[i, 0] + mass[j, 0]
            newdensity = (mass[i, 0] * density[i, 0] + mass[j, 0] * density[j, 0]) / (
                    mass[i, 0] + mass[j, 0])
            ...
            mass[jset, 0] = 1e-30
            position[jset] = rotate((randint(1e25, 1e30), 0, 0), angle=randint(0, 360) * pi / 180,
                                    axis=(0, 1, 0))
            if status[jset]:
                ct -= 1
    ...
    if scene.kb.keys:
        ...
        if k == 'b':
            ...
            objects.append(
                sphere(radius=objects[0].radius, pos=objects[0].pos, make_trail=True, interval=10,
            ...
            for n in range(len(objects)):
                if status[n]:
                    ...
                    velocity = sqrt(g * m / mag(position[n] - centerofmass)) * rotate(
                        norm(position[n] - centerofmass), angle=pi / 2,
                        axis=(0, 1, 0)) + centerofmassvelocity
        ...
        if k == 'k':
            ...
            status = resize(status, (len(status) + 1, 1))
            radius = resize(radius, (len(radius) + 1, 1))
            nposition = rotate(vector(20 * r, 0., 0.), angle=randint(0, 360) * pi / 180,
                               axis=(0, 1, 0)) + centerofmass
            mass[len(mass) - 1] = randint(1e4, 1e8)
            density[len(density) - 1] = randint(1e3, 1e4)
            position[len(position) - 1] = nposition
            velocity = randint(80, 120) / 100. * .25 * ct * rotate(-norm(nposition - centerofmass),
                                                                   angle=ct * randint(-5,
                                                                                      5) * pi / 180,
                                                                   axis=(0, 1, 0))
            ...
            objects.append(sphere(radius=sqrt(
                (mass[mass.__len__() - 1] * 3 / 4 / density[density.__len__() - 1] / pi)),
                pos=position[position.__len__() - 1], make_trail=True,
                interval=10))
            radius[len(radius) - 1] = objects[len(objects) - 1].radius
            radius = radius.ravel()
            if not textures:
        ...
        if k == 'a':
            ...
            nposition = rotate(vector(x, 0., 0.), angle=randint(0, 360) * pi / 180.,
                               axis=(0, 1, 0)) + centerofmass
            mass[len(mass) - 1] = randint(1, 1e5)
            density[len(density) - 1] = randint(5, 15)
            position[len(position) - 1] = nposition
            velocity = randint(80, 120) / 100. * sqrt(
                g * ct * mass[0] / mag(nposition - centerofmass)) * rotate(
                norm(nposition - centerofmass), angle=pi / 2, axis=(0, 1, 0)) + centerofmassvelocity
            momentum[len(momentum) - 1] = mass[len(mass) - 1] * velocity
            rotation[len(rotation) - 1] = randint(80, 120) / 100. * pi / 7500
            status[len(status) - 1] = False
            objects.append(sphere(radius=sqrt(
