...
class SolarSystem:
    def __init__(self, thesun):
        ...
        self.planets = []
        self.time = None
        self.timestamp = ax.text(.03, .94, 'Date: ', color='w', transform=ax.transAxes,
                                 fontsize='x-large')
...
for i, nasaid in enumerate([1, 2, 3, 4]):  # The 1st, 2nd, 3rd, 4th planet in solar system
    ...
    ax.text(0, - (texty[i] + 0.1), names[i], color=colors[i], zorder=1000, ha='center',
            fontsize='large')
...
ani = animation.FuncAnimation(fig, animate, repeat=False, frames=sim_duration, blit=True,
                              interval=20, )
plt.show()
