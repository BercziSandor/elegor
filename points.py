...
class Point(np.ndarray):
    ...
    # def __init__(self, shape, input_array=(0, 0)):
    ...
    def dist(self, other):
        ...
        """
        dis = np.linalg.norm(self - other)
        print(f"Distance between points {self} and {other}: {dis}")
        return dis


class Electron(Point):
    def __new__(cls, input_array=[0, 0]):
        obj = super.__new__(cls, input_array, q=1.6 * 10 ** -19)
        return obj
